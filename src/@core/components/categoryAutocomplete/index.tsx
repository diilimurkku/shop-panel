...
const CategoryAutoComplete = ({
  ...
  const { data: categoriesData, isLoading: isLoadingCategories } = useFetch().useQuery('get', '/select/{model}', {
    params: {
      path: {
        model: 'category'
      },
  ...
  return (
    <div className='flex items-center gap-x-2'>
      ...
      <FormDrawer ref={formDrawerRef} title={categoryTranslate?.addNewCategoryTitle ?? ''}>
        ...
        <Box display={'flex'} alignItems={'center'} columnGap={2} justifyContent={'end'} paddingX={5}>
          ...
          <LoadingButton variant='contained' type='submit' loading={isAddingCategory} onClick={handleSubmit(onSubmit)}>
